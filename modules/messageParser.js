...
{
    let file, subject, body;
    try
    {
        file = fs.readFileSync(path, 'utf8'); // получаем файл с сообщением из корневого каталога приложения
    } catch (error)
    {
    ...
    try
    {
        subject = file.match(/# .*/, "")[0].replace(/# /, "").trim(); // ищем "# ", которые предшествуют заголовку и сохраняем это как тему письма
        body = file.replace(/# .*/, "").trim(); // весь остальной контент используется как тело письма
    } catch (error)
    {
